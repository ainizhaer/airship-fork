...
public class LocalizationUtils: NSObject {
    ...
    private static func localizedString(
        ...
        let coreBundle = AirshipResources.bundle

        var string = sanitizedLocalizedString(
            string,
            withTable: table,
        ...
        if string == "" {
            if let fallbackLocale = fallbackLocale {
                ...
                let localizedMainBundle = Bundle(
                    path: mainBundle.path(
                        forResource: fallbackLocale,
                        ofType: "lproj"
                    )
                        ?? ""
                )
                let localizedCoreBundle = Bundle(
                    path: coreBundle.path(
                        forResource: fallbackLocale,
                        ofType: "lproj"
                    )
                        ?? ""
                )
                let localizedModuleBundle = Bundle(
                    path: moduleBundle!
                        .path(forResource: fallbackLocale, ofType: "lproj")
                        ?? ""
                )

                string = sanitizedLocalizedString(
                    string!,
                    withTable: table,
